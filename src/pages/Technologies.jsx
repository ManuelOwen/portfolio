...
const Technologies = () => {
  ...
  return (
    <div className="technologies font-medium text-xl w-11/12 mx-auto p-8 mt-10 text-gray-100 ">
      <hr className='mb-7'/>
      <h1 className="text-center text-blue-500 text-4xl mb-8">Technologies</h1>
      <div className="grid md:grid-cols-4 gap-3   ">
        {technologies.map((tech) => (
          <div key={tech.id} className="hover: p-5 rounded-lg text-center flex flex-col items-center mb-5 bg-gray-800 justify-center  transform transition-transform duration-300 hover:scale-105 pointer">
            <img src={tech.image} alt={tech.name} className="w-24 mb-4" />
            <h2 className="text-2xl text-blue-400">{tech.name}</h2>
            <p className="text-gray-300 my-5">{tech.description}</p>
          </div>
        ))}
      </div>
    </div>
